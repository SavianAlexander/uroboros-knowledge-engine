---
title: Neural Voice Subsystem Architecture & MCP Suite
category: System Architecture
tags: [Antigravity, MCP, Kokoro82M, WhisperSTT, PersonaBlending, VoiceMemory, FFTSpectrum, TududiRadar, AudioMastering]
last_updated: 2026-08-15
---

# Neural Voice Subsystem Architecture & Full-Duplex MCP Suite

This document defines the core voice subsystem architecture for Antigravity, unifying local neural speech synthesis, vector persona blending, Speech-to-Text (STT) ear transcription, hardware audio device routing, SQLite conversational memory, real-time FFT spectrum analysis, and autonomous Tududi task radar.
...
```mermaid
graph TD
    User["Antigravity Agent / Subagent / User"] --> MCP["Antigravity Voice MCP (src/antigravity_voice_mcp.py)"]
    
    subgraph Engines["Core Voice Subsystems"]
...
|---|---|---|
| `antigravity_speak` | Kokoro neural voice dispatcher with True-Peak limiter and DSP presets | `text`, `persona`, `speed`, `dsp_preset`, `priority`, `sfx_intro` |
| `antigravity_announce_task` | Milestone audio broadcaster | `task_name`, `state` (STARTED, COMPLETED, FAILED), `details` |
...
| `antigravity_get_spectrum` | 32-band log-frequency FFT spectrum for visualizer | `num_bands` |
...
## 3. Vector Persona Blending Formulation
...
The resulting vector $\mathbf{v}_{\text{blend}}$ is passed directly into the Kokoro-82M ONNX model, yielding a signature vocal timbre without retraining.
