...
def execute_process_hygiene_contract(bus: InterBridgeEventBus) -> BridgeContract:
    ...
    t0 = time.time()
    c_id = f"contract_process_hygiene_{int(time.time() * 1000)}"
    try:
        import process_hygiene_bridge
        clean_res = process_hygiene_bridge.clean_process_hygiene(dry_run=False)
        outputs = {
            "hygiene_status": clean_res.get("status", "success"),
            "terminated_count": len(clean_res.get("terminated_pids", [])),
            "reclaimed_memory_mb": clean_res.get("reclaimed_memory_mb", 0.0),
            "post_clean_hygiene_score": clean_res.get("post_clean_hygiene_score", "100%")
        }
        shared_context = {
            "os_process_hygiene_score": clean_res.get("post_clean_hygiene_score", "100%"),
            "os_orphans_cleared": len(clean_res.get("terminated_pids", []))
        }
        contract = BridgeContract(
            contract_id=c_id,
            bridge_name="process_hygiene_bridge",
            duration_ms=(time.time() - t0) * 1000,
            status="SUCCESS",
            outputs=outputs,
            shared_context=shared_context
...
def execute_file_allocation_contract(bus: InterBridgeEventBus) -> BridgeContract:
    """Executes File Allocation and Clean Architecture Topology scan, publishing Contract J."""
    t0 = time.time()
    c_id = f"contract_file_allocation_{int(time.time() * 1000)}"
    try:
        import file_allocation_bridge
        scan_res = file_allocation_bridge.scan_repository_allocation(repo_root=bus.repo_root)
        clean = scan_res.get("clean", False)
        outputs = {
            "file_allocation_status": "CLEAN" if clean else "WARNING",
            "total_violations": scan_res.get("total_violations", 0),
            "root_violations_count": len(scan_res.get("root_violations", [])),
            "misplaced_tests_count": len(scan_res.get("misplaced_tests", [])),
            "orphan_artifacts_count": len(scan_res.get("orphan_artifacts", []))
        }
        shared_context = {
            "file_allocation_clean": clean,
            "file_allocation_violations": scan_res.get("total_violations", 0)
        }
        contract = BridgeContract(
            contract_id=c_id,
            bridge_name="file_allocation_bridge",
            duration_ms=(time.time() - t0) * 1000,
