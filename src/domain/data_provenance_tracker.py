...
def track_data_provenance(
    ...
    if isinstance(file_content, bytes):
        content_bytes = unicodedata.normalize("NFC", file_content.decode("utf-8", errors="ignore")).encode("utf-8")
    elif isinstance(file_content, str):
        content_bytes = unicodedata.normalize("NFC", file_content).encode("utf-8", errors="ignore")
    elif file_content is not None:
        content_bytes = unicodedata.normalize("NFC", str(file_content)).encode("utf-8", errors="ignore")
    else:
        content_bytes = b""
