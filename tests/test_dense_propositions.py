...
def test_format_breadcrumb_scope():
...
def test_basic_proposition_decomposition():
...
def test_file_id_proposition_id_generation():
...
def test_markdown_heading_hierarchy_tracking():
    md_text = """
# System Architecture
...
def test_bullet_and_numbered_list_decomposition():
    list_text = """
...
def test_semicolon_compound_sentence_splitting():
...
def test_subminimal_fragment_filtration():
...
def test_abbreviation_and_decimal_protection():
...
def test_interrobang_and_ellipsis_handling():
...
def test_expand_propositions_to_parent_context_with_db():
...
def test_expand_propositions_to_parent_context_fallback():
