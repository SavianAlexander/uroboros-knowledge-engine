...
def test_retrieval_pipeline_creation_and_execution():
...
def test_rrf_score_fusion_stage():
...
def test_source_citation_stage():
    c = SearchCandidate(
        id=1,
        filepath="guide.md",
        filename="guide.md",
...
def test_consolidated_query_intent_engine():
...
def test_consolidated_reranking_engine():
...
def test_consolidated_graph_engine():
    graph_data = {
...
def test_consolidated_verification_guards():
...
def test_eve_infrastructure_package():
