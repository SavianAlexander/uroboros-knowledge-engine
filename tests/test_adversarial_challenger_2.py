...
PORT = 8092
DB_NAME = os.path.join(tempfile.gettempdir(), "test_challenger_2.db")
...
class TestAdversarialChallenger2(unittest.TestCase):
    port = PORT
    ...
    def setUpClass(cls):
        ...
        with get_db_connection(DB_NAME) as conn:
            ...
            cursor.execute(
                ...
                ("test_sandbox_challenger_2/file1.txt", "file1.txt", 100, "text/plain", "sha1")
            )
            file_id = cursor.lastrowid
            
            cursor.execute(
                "INSERT INTO files (filepath, filename, file_size, mime_type, sha256) VALUES (?, ?, ?, ?, ?)",
                ("test_sandbox_challenger_2/file2.txt", "file2.txt", 200, "text/plain", "sha2")
            )
