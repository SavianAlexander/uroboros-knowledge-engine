...
def query_episodic_rag(query: str, session_id: Optional[str] = None) -> Dict[str, Any]:
    ...
    # ponytail: session-aware episodic memory RAG; ceiling: 3 vector snippets + memory list merge; upgrade: add episodic memory decay weighting if session memory exceeds 1,000 entries
    """
    ...
    relevant_memories = []
    if query_terms and memories:
        for m in memories:
            if isinstance(m, dict) and m.get("key"):
                k_terms = set(str(m["key"]).lower().split("_"))
                if query_terms & k_terms:
